...
// TTS restaurado do CMIP antigo: áudio servidor primeiro, voz nativa apenas como fallback no cliente.
...
app.get('/api/tts', async (req, res) => {
  ...
  if (ttsCache.has(text)) return res.send(ttsCache.get(text));
  const providers = [
  ...
  for (const url of providers) {
    ...
    } catch (err) { console.warn('[TTS Provider Error]', err.message); }
  }
...
httpServer.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`\n==================================================`);
  console.log(`🚀 CMIP SERVIDOR DE SENHAS MULTI-TV RODANDO!`);
  console.log(`📍 Backend: http://localhost:${PORT}`);
  localIps.forEach(ip => {
    console.log(`   👉 TV Recepção:   http://${ip}:5173/tv-recepcao`);
    console.log(`   👉 TV 01 Térreo:  http://${ip}:5173/tv1`);
    console.log(`   👉 TV 02 1º Andar:http://${ip}:5173/tv2`);
    console.log(`   👉 Recepção/Guichê: http://${ip}:5173/`);
    console.log(`   👉 Painel Médico: http://${ip}:5173/medico`);
  });
  console.log(`==================================================\n`);
});
