...
import { 
  ...
  Send, RotateCcw, Tv, LogOut, ArrowRight, UserCheck, Shield 
} from 'lucide-react';
...
export default function ReceptionPanel({ user, onLogout, onNavigateTv }) {
  ...
  useEffect(() => {
    fetchOfficesAndDoctors().then(res => {
      if (res?.doctors) {
        setDoctors(res.doctors);
        if (res.doctors.length > 0) {
          setSelectedDoctorId(String(res.doctors[0].id));
        }
      }
      if (res?.offices) {
  ...
  const handleRegister = async (e) => {
    ...
    const officeName = selectedDoctor?.office_name || 'Consultório 01';
    ...
    if (res?.success) {
      ...
      setRecentPatients(prev => [newEntry, ...prev.slice(0, 7)]);
      setSuccessMsg(`Paciente ${patientName.trim()} enviado com sucesso para ${doctorName} (${officeName})!`);
    ...
    <div className="min-h-screen bg-cmip-950 text-slate-100 font-['Montserrat',sans-serif] p-4 md:p-8 cmip-plus-pattern relative">
      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 bg-cmip-900/90 border border-cmip-600/30 rounded-3xl shadow-2xl backdrop-blur-md">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[10px] font-black uppercase">
                  Recepção / Triagem Direta
                </span>
              </div>
              <h1 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight mt-1">
                Cadastro Imediato de Pacientes
              </h1>
              <p className="text-xs text-cmip-100/70">Encaminhamento direto para a fila do médico sem intermediários</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span>Sair ({user?.name || 'Recepção'})</span>
            </button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 bg-cmip-900/80 border border-cmip-600/30 p-8 rounded-3xl glass-panel space-y-6 shadow-2xl">
            ...
            <div className="flex items-center gap-3 border-b border-cmip-600/30 pb-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Cadastrar Paciente na Fila</h2>
                <p className="text-xs text-cmip-100/70">O paciente será adicionado em tempo real na tela do médico</p>
              </div>
            </div>
            ...
            <form onSubmit={handleRegister} className="space-y-5">
              ...
              <div>
                <label className="block text-xs font-bold text-cmip-100 uppercase tracking-wider mb-2 flex items-center gap-2">
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-cmip-100/70 mt-0.5">
                        Destino: <strong className="text-cmip-400">{p.doctor}</strong> ({p.office})
                      </div>
                    </div>
