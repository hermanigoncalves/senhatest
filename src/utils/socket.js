  ...
  try {
    ...
    try {
      ...
      if (!res.ok) {
        return { 
          success: false, 
          message: `Servidor indisponível (HTTP ${res.status}). Certifique-se de que o backend 'node server.js' está em execução na porta 3001.` 
        };
      }
      return { 
        success: false, 
    ...
    return { 
      success: false, 
      message: 'Não foi possível conectar ao servidor backend (porta 3001). Inicie o servidor com "node server.js" ou "npm run server".' 
    };
  }
}
...
export async function verifySession() {
  try {
    ...
    try {
      ...
      return { success: false, message: 'Falha ao validar sessão.' };
    }
  } catch (e) {
    return { success: false, message: e.message };
  }
